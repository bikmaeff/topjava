...
public class JdbcUserMealRepositoryImpl implements UserMealRepository {
    ...
    public UserMeal save(UserMeal userMeal, int userId) {
        ...
        if (userMeal.isNew()) {
            Number newId = insertUserMeal.executeAndReturnKey(mapSqlParameterSource);
    ...
    public List<UserMeal> getAll(int userId) {
        return jdbcTemplate
                .query("SELECT * FROM meals WHERE user_id = ? ORDER BY datetime DESC ", ROW_MAPPER, userId);
    }

    @Override
    public List<UserMeal> getBetween(LocalDateTime startDate, LocalDateTime endDate, int userId) {
        return jdbcTemplate.
                query("SELECT * FROM meals WHERE user_id=?  AND datetime BETWEEN  ? AND ? ORDER BY datetime DESC",
